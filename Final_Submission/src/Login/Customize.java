...
public class Customize extends JFrame{
	...
	public int countH = 0,countB =0,countS=0,countW=0;
	...
	public int setBodyIdx(String s){
		if(s.equals(null)) bodyIndex= 0;;
	...
	public int setShirtIdx(String s){
		if(s.equals(null)) shirtIndex=0;
	...
	public int setHeadIdx(String s){
	...
	}public int setWeaponIdx(String s){
		if(s.equals(null)) return 0;
		//int index;
		for(int i = 0;i<weapon.length;i++){
			if(s.equals(weapon[i])){
				return  i;
			}
		}
	...
	public Customize() throws IOException 
	{
		...
		txtpnHeadGear.setEditable(false);
	...
	private void showBody() throws IOException {
		if(countB==0){ 
			bodyIndex = setBodyIdx(GridProjection.userLook.gidB);
	...
	private void showShirt() throws IOException {
		if(countS==0) {
			shirtIndex = setShirtIdx(GridProjection.userLook.gidS);
			shirtIcon  = getShirt(shirtIndex);
		}else shirtIcon  = getShirt(shirtIndex);
	...
	private void showHead() throws IOException {
		if(countH==0) 
			{
			headIndex = setHeadIdx(GridProjection.userLook.gidH);
			headIcon  = getHead(headIndex);
			}else headIcon  = getHead(headIndex);
	...
	private void showWeapon() throws IOException {
		if(countW==0) {
			weaponIndex = setWeaponIdx(GridProjection.userLook.gidW);
			weaponIcon  = getWeapon(weaponIndex);
		}else weaponIcon  = getWeapon(weaponIndex);
	...
	public class ButtonListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			if(source==bUp){
				...
				a.gidS = shirt[shirtIndex];
				a.gidW = weapon[weaponIndex];
				
				GridProjection.userLook = a;
				GridProjection.ubImage = bodyIcon.getImage();
				GridProjection.uwImage = weaponIcon.getImage();
				GridProjection.uhImage = headIcon.getImage();
				GridProjection.usImage = shirtIcon.getImage();
				
				test1.immigration.customization(a);
				dispose();
